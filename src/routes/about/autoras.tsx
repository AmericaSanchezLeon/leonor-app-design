...
function SocialIcon({ href, label, badge, children }: { href: string; label: string; badge?: boolean; children: React.ReactNode }) {
  return (
    <a
      ...
      className={
        badge
          ? "flex h-7 w-7 items-center justify-center rounded-md bg-foreground text-background transition hover:opacity-80"
        ...
        {autoras.map((a, i) => {
          ...
          return (
            <article key={i} className="flex flex-col items-center text-center">
              ...
              <p className="font-serif text-2xl font-semibold" style={{ color: "var(--section-color)" }}>
                {a.autora}
              </p>
              {a.descripcion && (
                <p className="mt-4 text-sm leading-relaxed">{a.descripcion}</p>
              )}
